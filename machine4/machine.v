...
Definition copy_out_of_band_trans (c : Config) (s1 s2 : Site)
  (q3 q4 : queue Message) :=
  mkconfig (st c) (rt c)
    (change_queue (queue Message) (bm c) s1 s2
       (append Message q3 (input Message copy q4))).
...
Inductive class_trans (c : Config) : Set :=
  ...
  | redirect_inc :
      ...
      class_trans c
  | copy_out_of_band :
      forall (s1 s2 : Site) (q1 q2 q3 q4 : queue Message),
      s1 <> s2 -> (* this condition is introduced to facilitate the proof.  
                          I was unable to derive it myself *)
      bm c s1 s2 = append Message q1 (input Message copy q2) ->
      append Message q1 q2 = append Message q3 q4 -> class_trans c.
...
Definition transition (c : Config) (t : class_trans c) :=
  ...
  | copy_out_of_band s1 s2 q1 q2 q3 q4 h1 h2 h3 =>
      copy_out_of_band_trans c s1 s2 q3 q4
  end.
